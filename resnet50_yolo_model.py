...
class ResNet(nn.Module):
    def __init__(self, ResBlock, layer_list, cfg, num_channels=3):
        ...
        self.fc = nn.Sequential(
            nn.Linear(512*ResBlock.expansion, cfg.num_hidden),
            nn.Dropout(0.0),
            nn.LeakyReLU(0.1),
    ...
    def forward(self, x):
        ...
        x = self.fc(x).view(x.size(0), self.S, self.S, self.C + self.B * 5)
...
def ResNet50(cfg, channels=3):
    return ResNet(Bottleneck, [3, 4, 6, 3], cfg, channels)
